...
const VoiceAssessment = ({ difficulty, sessionActive, type = 'hr' }) => {
    const { jobRole } = useAssessment();
    const { resumeData, sessionsHistory } = useGlobal();

    // Resume Level Logic (0-100)
    const resumeLevel = resumeData?.totalScore || 50;
    const previousSessionCount = sessionsHistory ? sessionsHistory.length : 0;

    const [mode, setMode] = useState('low');
    const [isListening, setIsListening] = useState(false);
    const [transcript, setTranscript] = useState('');

    // Initialize with a smarter question
    const [question, setQuestion] = useState(() =>
        generateQuestion(jobRole, resumeLevel, sessionsHistory, type)
    );
        ...
        <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 border border-white/10 rounded-xl p-1 h-full flex flex-col shadow-2xl relative overflow-hidden">
            ...
            <div className="bg-slate-950/50 backdrop-blur-3xl rounded-lg p-3 h-full flex flex-col relative z-10 w-full">
                ...
                <div className={`flex-grow relative rounded-xl mb-3 overflow-hidden transition-all duration-500 border border-white/5 flex flex-col md:flex-row ${isListening ? 'bg-indigo-900/10' : 'bg-slate-900/50'}`}>
                    ...
                    {/* Side Panel Question Text - Only for HR */}
                    {type !== 'technical' && (
                ...
                {/* Unified Controls & Transcript Bar - Only for HR */}
                {type !== 'technical' && (
                    <div className="bg-black/20 rounded-xl p-1.5 flex items-center gap-2 border border-white/5 backdrop-blur-sm shrink-0 h-14 relative z-20">
                        ...
                        <div className="flex flex-col gap-0.5">
                            <button
                                className="p-1.5 text-emerald-400 hover:bg-white/5 rounded-md transition-colors"
                                title="Next Question"
                                onClick={() => {
                                    const newQ = generateQuestion(jobRole, resumeLevel, sessionsHistory, type);
                                    setQuestion(newQ);
                                    setTranscript('');
                                }}
                            >
