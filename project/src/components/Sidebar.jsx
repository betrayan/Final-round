...
const Sidebar = ({ isOpen, setIsOpen }) => {
    ...
    return (
        <>
            ...
            <aside className={`fixed lg:sticky top-0 h-screen w-[220px] sm:w-[240px] md:w-[260px] backdrop-blur-xl border-r z-40 transition-all duration-300 ease-out flex flex-col shadow-2xl ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} ${isDark ? 'bg-slate-950/95 border-white/10' : 'bg-white/95 border-slate-200'}`}>
                ...
                {/* Logo Section */}
                <div className={`p-4 sm:p-5 md:p-6 border-b backdrop-blur-sm relative z-10 ${isDark ? 'border-white/10' : 'border-slate-200'}`}>
                    <div className="flex items-center justify-between">
                        ...
                        <button onClick={() => setIsOpen(false)} className={`lg:hidden transition-colors p-1.5 rounded-lg ${isDark ? 'text-slate-400 hover:text-white hover:bg-white/5' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-100'}`}>
                            <X size={20} />
                        </button>
                    </div>
                </div>
                ...
                <nav className="flex-grow px-2 sm:px-3 py-4 sm:py-5 space-y-5 sm:space-y-6 overflow-y-auto scrollbar-thin relative z-10">
                    ...
                    <div className={`${!isAssessmentActive ? `pt-3 border-t ${isDark ? 'border-white/10' : 'border-slate-200'}` : ''}`}>
                        <div className="px-3 mb-3 flex items-center gap-2">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <NavItem to="/ml-readiness" icon={Target} label="ML Prediction" />
                        </div>
                    </div>
                </nav>
                ...
                {!isAssessmentActive && (
                    <div className={`p-3 border-t z-10 ${isDark ? 'border-white/10' : 'border-slate-200'}`}>
...
const NavItem = ({ to, icon: Icon, label }) => {
    ...
    <NavLink
        to={to}
        className={({ isActive }) => `flex items-center gap-3 px-3 py-3 rounded-xl font-medium text-sm transition-all duration-300 group relative overflow-hidden ${isActive
            ? `border shadow-lg ${isDark ? 'text-white bg-gradient-to-r from-emerald-500/20 to-teal-500/20 border-emerald-500/30 shadow-emerald-500/10' : 'text-emerald-700 bg-emerald-50 border-emerald-200 shadow-emerald-500/5'}`
            : `border border-transparent ${isDark ? 'text-slate-300 hover:text-white hover:bg-white/5 hover:border-white/10' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100 hover:border-slate-200'}`
        }`}
    >
        {({ isActive }) => (
            <>
